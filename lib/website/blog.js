...
async function readCatalog() {
  ...
  if (!response?.content) {
    throw new Error('GitHub did not return blog/posts.json content.');
  }
  ...
  if (!Array.isArray(posts)) {
    throw new Error('blog/posts.json is not a valid array.');
  }
...
// Upload an image sent through Telegram into the same GitHub website repo.
async function uploadImage(
  ...
  if (buffer.length > 8 * 1024 * 1024) {
    throw new Error(
      'Image is too large. Please send a smaller image.'
    );
  }

  const match = String(originalName)
    .toLowerCase()
  ...
  const filename =
    `telegram-${Date.now()}-${Math.random().toString(36).slice(2, 8)}.${safeExt}`;

  const mediaPath = `blog-media/${filename}`;

  await request(
    'PUT',
    `/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/contents/${mediaPath}`,
    {
...
async function updatePost(id, patch) {
  ...
  await writePosts(
    posts,
    sha,
    `Update blog post: ${updated.title}`
  );
...
async function deletePost(id) {
  ...
  await writePosts(
    posts,
    sha,
    `Delete blog post: ${removed.title || removed.id}`
  );
