...
web3Automation.setPromotionHandler(async ({ title, description, url }) => {
  ...
  try {
    ...
    return { sent: results.filter(x => x.status === "sent").length, failed: results.filter(x => x.status === "failed").length };
  } finally {
    if (previous?.id) {
      try { await setActiveCampaign(previous.id); } catch (e) { console.error("Could not restore previous active campaign:", e.message); }
    }
  }
});
...
async function showWeb3AutomationMenu(chatId, messageId) {
  ...
  const rows = [
    ...
    [btn("📰 Publish News Now", "web3_news_now")],
    [btn("🎯 Publish Opportunity Now", "web3_publish_now")],
    [btn("🔄 Refresh", "menu_web3_automation")],
    [btn("⬅️ Main Menu", "menu_main")]
  ];
...
bot.on("callback_query", async q => {
  ...
  try {
    ...
    if (data === "web3_toggle") {
      ...
      await web3Automation.saveSettings({ enabled: !current.enabled });
    ...
    if (data === "web3_news_now") {
      ...
      const result = await web3Automation.publishNews();
      return safeEdit(chatId, messageId, `✅ News article published.\n\n${result.post.title}\n${result.post.url}`, { inline_keyboard: [[btn("🤖 Automation", "menu_web3_automation")]] });
    }
    if (data === "web3_publish_now") {
      await safeEdit(chatId, messageId, "🎯 Selecting a stored opportunity and publishing it...", { inline_keyboard: [] });
